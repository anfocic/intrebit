...
type Target = string | Element | Array<Element> | NodeListOf<Element>;
...
const defaults: Required<
    Omit<
        ...
        "root" | "title" | "text" | "extra" | "onComplete"
    >
> & {
    onComplete?: () => void;
} = {
...
function toElements(target: Target | undefined, root: ParentNode): Element[] {
    if (!target) return [];
...
export function createRevealSequence(
    ...
    const titleEls = toElements(opts.title, root);
    const textEls = toElements(opts.text, root);
    const extraEls = toElements(opts.extra, root);

    // Reduced motion: set to final state and bail
    if (prefersReducedMotion()) {
        const all = [...titleEls, ...textEls, ...extraEls];
        if (all.length) {
            gsap.set(all, {
                opacity: 1,
                y: 0,
    ...
    const killAllTweens = (els: Element[]) => {
        for (const el of els) gsap.killTweensOf(el);
    };
    ...
    return {
        destroy: () => {
            timeline?.kill();
            timeline = null;
            killAllTweens(titleEls);
            killAllTweens(textEls);
            killAllTweens(extraEls);
        },
        replay: () => {
            timeline?.kill();
            timeline = null;
